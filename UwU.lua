...
function library:New(name) 
	...
	Menu.Image = "http://www.roblox.com/asset/?id=8015528530" 
...
local gui = library:New("PhuphaSense                                                                                          ") 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "w i n n i n g" or "$$$", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"Winning", "PhuphaSense"}}) 
